...
export const ComunidadQuery = async (id_comunidad:number): Promise<any> => {
    try {
...
export const verComunidadQuery = async (id_comunidad: number): Promise<any> => {
    try {
        const response: QueryResult = await pool.query('SELECT * FROM comunidades WHERE id_comunidad=$1', [id_comunidad]);
        console.log(response);
        const multimedia: QueryResult = await pool.query('SELECT m.id_multimedia, m.ruta, m.tipo_Archivo FROM multimedias m where m.tipo = 1 AND m.estado = 1 AND m.id_tabla_asociado = $1', [id_comunidad]);
        //console.log('multimedia', multimedia);

        const comunidad = response.rows[0];

        comunidad.multimedias = multimedia.rows;

        let resp = {
            ok: true,
            msg: 'Se recupero el registro con exito',
            data: comunidad //response.rows[0]
        }       
        return resp;
    } catch (error) {
        const resp = {
...
export const createComunidadQuery = async (nombre: string, descripcion: string, superficie: number, poblacion: number, longitud: number, latitud: number, estado: number, id_usuario: number): Promise<any> => {
    ...
    try {
        const response: QueryResult = await pool.query('INSERT INTO comunidades(nombre,descripcion,superficie,poblacion,longitud,latitud,estado,id_usuario) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id_comunidad', [nombre, descripcion, superficie, poblacion, longitud, latitud, estado, id_usuario]);
        let resp = {
            ok: true,
            msg: 'Se agrego el registro con exito',
            data: response
